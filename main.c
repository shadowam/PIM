...
void adicionar_livro()
{
    ...
    do {
        ...
        to_maiuscula(book[IDcont].local, 30, 36);
        gotoxy(30,37);
...
void alterar(int id)
{
    ...
    textcolor(LIGHTGREEN);
    printf("Pressione ENTER para iniciar alteracao ou ESC para voltar.");
    textcolor(WHITE);
    gotoxy(120,47);
            ...
            do
            {
                ...
                if (setas==ENTER)
                {
                    ...
                    else
                    {
                        ...
                        fprintf(arq, "%d:%s:%s:%s:%s:%d:%d:%d:%s:%s\n", livro[id].ID, livro[i].titulo, livro[i].autor, livro[i].genero, livro[i].editora, livro[i].edicao, livro[i].ano, livro[i].paginas, livro[i].ISBN, livro[i].local);
                        while (book[i+1].ID != 0)
                        {
